...
app.post('/register/empresa', (req, res) => {
  ...
  db.beginTransaction(err => {
    ...
    const insertUserQuery = 'INSERT INTO usuario (email, password, telefone, provincia, area_atuacao) VALUES (?, ?, ?, ?, ?)';
    db.query(insertUserQuery, [email, password, telefone, provincia, area_atuacao], (err, result) => {
      if (err) {
      ...
      const insertCompanyQuery = 'INSERT INTO empresa (user_id, nif_empresa, nome_empresa, nome_representante, ano_existencia) VALUES (?, ?, ?, ?, ?)';
      db.query(insertCompanyQuery, [userId, nif_empresa, nome_empresa, nome_representante, ano_existencia], (err, result) => {
        if (err) {
...
// Endpoint para registrar empreendedor
...
app.put('/update/profile/empresa', (req, res) => {
  ...
  db.beginTransaction(err => {
    ...
    const updateUserQuery = 'UPDATE usuario SET email = ?, telefone = ?, provincia = ?, area_atuacao = ? WHERE id = ?';
    db.query(updateUserQuery, [email, telefone, provincia, areaAtuacao, userId], (err, result) => {
      if (err) {
      ...
      db.query(updateCompanyQuery, [nif_empresa, nome_empresa, nome_representante, ano_existencia, userId], (err, result) => {
        ...
        db.commit(err => {
          ...
          res.status(200).send({ message: 'Perfil de empresa atualizado com sucesso' });
        });
      });
    });
  });
    ...
    db.query(updateUserQuery, [email, telefone, provincia, areaAtuacao, userId], (err, result) => {
      ...
      db.query(updateEntrepreneurQuery, [bi, nome, data_nascimento, genero, userId], (err, result) => {
        ...
        db.commit(err => {
          ...
          res.status(200).send({ message: 'Perfil de empreendedor atualizado com sucesso' });
        });
      });
    });
  });
